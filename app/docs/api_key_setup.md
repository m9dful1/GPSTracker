...
The app reads its Google Maps/Places/Routes API key from `local.properties`
...
5. Under "Application restrictions": note that this app calls the Routes API
   over HTTP, which does not send Android app credentials. An
   Android-app-restricted key will work for the Maps/Places SDKs but the
   Routes API calls will be denied. For development, leave application
