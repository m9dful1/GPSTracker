...
- **"Use mobile data" setting honored**: the settings switch existed and persisted but nothing read it — bulk content prefetch (up to 10 Wikipedia fetches per geofence refresh) ran on cellular regardless. `ContentServiceImpl.prefetchContent` now skips on metered networks unless the user opted in (`ConnectivityChecker` via `ConnectivityManager.isActiveNetworkMetered`); on-demand geofence-triggered narration fetches are deliberately unaffected so the core feature keeps working on the road.
- **Night-mode map**: the map follows the system dark theme via Google's night `MapStyleOptions` (`res/raw/map_style_night.json`) applied in `onMapReady` — the rest of the UI already used a DayNight theme, the map was the one full-brightness hole during night drives. Also: edge-anchored controls now add the system-bar insets on top of their design margins (targetSdk 35 edge-to-edge left FABs under the 3-button nav bar), and the map gets matching padding for Google's logo/compass.
- **Shareable journal**: a share button on the Tour Journal exports it as plain text ("My tour journal — 12 places discovered: • Fort Point (Historical) — Jul 7 at 2:14 PM ...") through the Android share sheet. `JournalFormatter.shareText` is pure — date rendering is injected by the caller — and the button disables while the journal is empty.
...
- **Long-press to scout**: holding a finger anywhere on the map discovers POIs around that point (750 m, shown with a green circle outline) — preview a destination before driving there. Scouting doesn't touch `lastPoiFetchCenter`, so the results persist while parked; moving ≥300 m lets the regular around-me refresh reclaim the map and remove the circle.
...
- **"Max notifications per hour" setting honored**: the settings slider persisted but nothing read it — a dense corridor could narrate nonstop regardless. `TourLogic.narrationAllowed` (pure, sliding one-hour window) now gates `generateAndQueueContent`; only narrations the delivery queue accepts count against the cap, and skipped places stay unvisited so they're eligible again once the hour rolls over. Sliding to 0 mutes automatic narration entirely — spelled out in the settings label ("0 per hour — automatic narration off") — while on-demand playback from the details sheet is unaffected.
- **The guide's memory fades**: never-repeat-a-place meant a daily commute went permanently silent after the first week — the app muted itself on exactly the routes driven most. `TourLogic.shouldSkipNarration` now applies a 14-day revisit cooldown: recently narrated places stay quiet, older ones become eligible again (still priority-penalized below fresh places, and still shown faded/journaled as visited). `mergeVisitedState` carries the visit timestamp onto fresh API results (DAO `VisitedPlaceRecord` projection) so both the geofence and proximity paths can evaluate the cooldown, and narration completion re-saves with a fresh timestamp so a re-heard place restarts its cooldown instead of repeating every pass. Visited places with no timestamp stay skipped.
- **Tour mode finally reads the saved settings**: `TourModeService` ran on `UserPreferences()` factory defaults 100% of the time — MainActivity started it with a bare intent action, `startTourMode`'s default parameter filled in defaults, and the geofence-revival path passed `UserPreferences()` explicitly. Every setting the service consults (voice speed/pitch, audio on/off, detail level, categories, notify distance, mobile data, narration cap) was silently ignored in the tour path even after being individually wired up. The service now injects `UserPreferencesRepository`, loads the saved settings at tour start (including after process-death revival), and keeps collecting the flow so settings edits apply mid-tour without restarting — TTS rate/pitch update live via the idempotent `AudioService.initialize`. The collector drops the initial DataStore replay to avoid double-initializing TTS at startup.
- **Distance in the narration intro**: "On your right, about 200 meters: Fort Point." — at speed the geofence fires up to 1.6 km out, so direction alone points at something the driver won't see for a minute; the distance says when to look. `TourLogic.distancePhrase` rounds for speech (nearest 50 m under 350 m, nearest 100 m under 950 m, nearest half-kilometer beyond; silent within 75 m where a callout is noise) since GPS/geofence jitter makes precision fake. A stationary user keeps the neutral "Coming up" intro but still hears the distance — what a parked scout wants to know.
- **Map layers sheet**: the layers FAB used to flip traffic AND advance the map type in one tap (with a toast that only mentioned traffic) — two unrelated changes per press. It now opens `MapLayersBottomSheet`, Google-Maps style: map type is an explicit choice (Default / Satellite / Terrain) and traffic is an independent switch, both applying immediately. "Satellite" maps to `MAP_TYPE_HYBRID` (imagery with road labels — unlabeled satellite is useless for wayfinding), and a lingering plain-satellite state from the old cycle is recognized when restoring the checked option. The built-in top-right my-location button is hidden — it duplicated the recenter FAB (which also re-engages camera following).
- **Layer choices survive restarts**: the layers-sheet selections persist in DataStore (`map_type`, `map_traffic` — kept out of `UserPreferences`, which is tour behavior, not map display) and are restored in `onMapReady`. `normalizeMapType` sanitizes stored values so a `MAP_TYPE_NONE` or a foreign version's constant can never restore into a blank map grid.
